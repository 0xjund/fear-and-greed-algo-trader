...
# Double check WBTC
STARTING_ETH_BALANCE = int(1e18)
STARTING_WBTC_BALANCE = int(1e18)
STARTING_USDC_BALANCE = int(100e6)

def _add_eth_balance():
    boa.env.set_balance(boa.env.eoa, STARTING_ETH_BALANCE)

def _add_token_balance(usdc: ABIContract, weth: ABIContract, activate_network: Network):
    print(f"USDC balance before: {usdc.balanceOf(boa.env.eoa)}")
    weth.deposit(value=STARTING_WBTC_BALANCE)
...
def setup_script() -> Tuple[ABIContract, ABIContract, ABIContract, ABIContract]:
    ...
    if active_network.is_local_or_forked_network():
        _add_eth_balance()
        _add_token_balance(usdc, wbtc, active_network)
